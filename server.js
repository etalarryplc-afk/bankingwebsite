...
app.post('/api/accounts', (req, res) => {
    ...
    const { accountType, firstName, lastName, idNumber, phoneNumber, zipCode, city, documentInfo } = req.body;
    ...
    verifyUser(token, (user) => {
        ...
        // Parse document info if provided
        let docInfo = null;
        if (documentInfo) {
            try {
                docInfo = typeof documentInfo === 'string' ? JSON.parse(documentInfo) : documentInfo;
            } catch (e) {}
        }
        
        db.run(`INSERT INTO accounts (user_id, account_type, account_number, id_number, status, balance, id_front, id_back) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [user.id, accountType, accountNumber, idNumber, 'pending', 0, docInfo ? docInfo.frontId.name : null, docInfo ? docInfo.backId.name : null],
            function(err) {
                if (err) {
